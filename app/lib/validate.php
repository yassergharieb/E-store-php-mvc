                ...
                foreach ($validationRoles as $validationRole) {
                    ...
                    if(preg_match_all('/(min)\((\d+)\)/', $validationRole, $m)) {
                        ...
                        if($this->min($value, $m[2][0]) === false) {
                            $this->messenger->add(
                                $this->langauge->feedKey('text_error_'.$m[1][0], [$this->langauge->get('label_'.$fieldName), $m[2][0]]),
                                Messenger::DANGER_COLOR
                            );
                        ...
                        if($this->between($value, $m[2][0], $m[3][0]) === false) {
                            $this->messenger->add(
                                $this->langauge->feedKey('text_error_'.$m[1][0], [$this->langauge->get('label_'.$fieldName), $m[2][0], $m[3][0]]),
                                Messenger::DANGER_COLOR
                            );
                            $errors[$fieldName] = true;
                        }
                        ...
                        if($this->eq($value, $m[2][0]) === false) {
                            $this->messenger->add(
                                $this->langauge->feedKey('text_error_'.$m[1][0],
                                 [$this->langauge->get('label_'.$fieldName), $m[2][0]]),
                                Messenger::DANGER_COLOR
                            );
                            $errors[$fieldName] = true;
                        }
                        ...
                        if($this->eq_field($value, $otherFieldValue) === false) {
                            $this->messenger->add(
                                $this->langauge->feedKey('text_error_'.$m[1][0],
                                 [$this->langauge->get('label_'.$fieldName),
                                 $this->langauge->get('label_'.$m[2][0])]),
                                Messenger::DANGER_COLOR
                            );
                            $errors[$fieldName] = true;
                        }
                        ...
                        if($this->$validationRole($value) === false) {
                            $this->messenger->add(
                                $this->langauge->feedKey('text_error_'.$validationRole, [$this->langauge->get('label_'.$fieldName)]),
                                Messenger::DANGER_COLOR
