...
class SessionManager extends SessionHandler
{
    ...
    private $sessionPath = '/';
    private $sessionDomain = '.mvcoop.com';
    ...
    public function __get($key) {
        if(isset($_SESSION[$key])) {
            $data = @json_decode((string)$_SESSION[$key]);
            if(is_object($data)) {
                return $data;
            } 
            return $_SESSION[$key];
    ...
    public function readsesession($id)


    {

    
        return openssl_decrypt(SessionHandler::read($id), $this->sessionCipherAlgo, $this->sessionCipherKey);
    }
